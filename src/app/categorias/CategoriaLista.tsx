...
import { eliminarCategoriaAction, guardarCategoriaAction } from '../actions/categoriaActions';
...
export default function CategoriaLista({ categoriasInitial }: { categoriasInitial: Categoria[] }) {
  // --- ESTADOS ---
  ...
  const filteredCategorias = useMemo(() => {
    return categoriasInitial.filter(cat => 
      cat.Nombre.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, categoriasInitial]);

  // Calcular el total de páginas
  const totalPages = Math.ceil(filteredCategorias.length / itemsPerPage);
  
  // Manejar la paginación de categorías
  ...
  const handleOpenModal = (categoria: Categoria | null = null) => {
    ...
    setPreviewUrl(categoria?.LogoUrl || null);
    setIsModalOpen(true);
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Categorías</h1>
          <p className="text-slate-500 text-sm">Gestiona las agrupaciones de tus productos</p>
        </div>
        <button 
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form action={async (formData) => {
                await guardarCategoriaAction(formData, selectedCategoria?.Id);
                setIsModalOpen(false);
                toast.success(selectedCategoria ? 'Cambios guardados' : 'Categoría creada');
              ...
              <div>
                ...
                <input 
                  name="logoUrl" 
                  defaultValue={selectedCategoria?.LogoUrl || ''} 
